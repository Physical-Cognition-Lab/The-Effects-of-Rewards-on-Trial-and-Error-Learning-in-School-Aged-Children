...
Time = read_sav('..\\..\\Data\\Databases\\DatabaseSRATTTIME_2.sav')
Switch = read_sav('..\\..\\Data\\Databases\\databasetoolswitching.sav')
Distance = read_sav('..\\..\\Data\\Databases\\databasedistance.sav')
Ntools = read_sav('..\\..\\Data\\Databases\\databasenbtool.sav')
...
Time.base.sr = lm(sr~1, data = Time)
...
# Bayes Factor
BF.sr = test_bf(Time.base.sr, Time.mod2.sr)
...
T1.1 = Time %>%
  ...
  ggplot( aes(x=age, y=Predicted, color = group))+
    ...
    geom_text( inherit.aes = F, data =slopePlot, aes(x = 10, y=100, label = P), size = 18)+
    facet_wrap(~ group, labeller = labeller(group = 
      c('1' = 'Positive reward group',
        '2' = 'Negative reward group',
        '3' = 'Control')))+
...
T1.2 = slopePlot %>%
  ...
  geom_errorbar(aes(ymin = CI_low, ymax = CI_high),  width =0.1, alpha =0.3,linewidth=1.5)+
  geom_pointrange(aes(ymin = Coefficient -SE, ymax = Coefficient +SE),size=2.4, linewidth=1.5)+
  theme_classic(base_size = 40)+
  theme(axis.line = element_blank(),legend.position = "bottom", axis.text.x = element_text(hjust = 0.5))+
  scale_x_discrete(name=NULL,label = c('Positive\nreward group','Negative\nreward group','Control'))+
...
T2 = ggplot(Time, aes(x=group, y = timeperattempt, color = group ))+
  ...
  scale_colour_manual(values=palette1)+
  scale_x_discrete(label = c('Positive\nreward group','Negative\nreward group','Control'))+
  scale_y_continuous(labels=function(x)x/1000)+
  labs(y = "Attempt time (sec)")+
  coord_cartesian(xlim = c(1.13, 2.9))+
  theme_classic(base_size = 40)+
  theme(axis.line = element_blank(),legend.position = "none",axis.text.x = element_text(hjust = 0.5),
...
```{r, message=FALSE, fig.height=10, fig.width=10, fig.align='center', dpi = 300, warning=FALSE}
##### Model the data using simple linear model
Time.base.attempttr = lm(attemptpertrial~1, data =  Time )
...
# Bayes Factor
BF.attempttr = test_bf(Time.base.attempttr, Time.mod2.attempttr)
...
T3.1 = Time %>%
  ...
  ggplot( aes(x=age, y=Predicted, color = group))+
    ...
    geom_text( inherit.aes = F, data =slopePlot, aes(x = 10, y=5.8, label = P), size = 18)+
    facet_wrap(~ group, labeller = labeller(group = 
      c('1' = 'Positive reward group',
        '2' = 'Negative reward group',
        '3' = 'Control')))+
...
```{r, message=FALSE, fig.height=10, fig.width=10, fig.align='center', dpi = 300, warning=FALSE}
##### Model the data using simple linear model
Switch.base = lm(toolswitching ~ 1, data = Switch)
...
# Bayes Factor
BF.switch = test_bf(Switch.base, Switch.mod2)
...
```{r, message=FALSE, fig.height=10, fig.width=10, fig.align='center', dpi = 300, warning=FALSE}
##### Model the data using simple linear model
Ntools.base = lm(nbtool~1, data =  Ntools )
...
# Bayes Factor
BF.ntools = test_bf(Ntools.base, Ntools.mod2)
...
```{r, message=FALSE, fig.height=10, fig.width=10, fig.align='center', dpi = 300, warning=FALSE}
##### Model the data using simple linear model
Distance.base = lm(distance~1, data =  Distance )
...
# Bayes Factor
BF.distance = test_bf(Distance.base, Distance.mod2)
...
ggsave('..\\..\\Results\\Plot\\FinalPlot_Time.jpg',comb1 , width = 1920*3, height = 1080*8, units = "px", dpi = 300)
ggsave('..\\..\\Results\\Plot\\FinalPlot_Time.svg',comb1 , width = 1920*3, height = 1080*8, units = "px", dpi = 300)
ggsave('..\\..\\Results\\Plot\\FinalPlot_Time.tiff',comb1 , width = 1920*3, height = 1080*8, units = "px", dpi = 300)
```
...
ggsave('..\\..\\Results\\Plot\\FinalPlot_Int.jpg', comb2, width = 1920*3, height = 1080*8, units = "px", dpi = 300)
ggsave('..\\..\\Results\\Plot\\FinalPlot_Int.svg', comb2, width = 1920*3, height = 1080*8, units = "px", dpi = 300)
ggsave('..\\..\\Results\\Plot\\FinalPlot_Int.tiff', comb2, width = 1920*3, height = 1080*8, units = "px", dpi = 300)
